...
## 4. Later CodeRabbit reviews (commits `23b8f2a`, `3323423`)

| # | Finding | Disposition |
|---|---|---|
| 6 | `ToolRun.output` concatenated `stdout + stderr` with no separator | **Applied** (`3323423`). Real: about 47 substring assertions across 11 modules read `.output`, several of them negative (`... not in run.output`). With no separator, a tool whose last stdout line carries no terminator fuses it with stderr's first line, yielding a string present in neither stream — which can satisfy a positive assertion or wrongly break a negative one. The two are now joined with a newline when stdout does not already end with one; both stream contents are unchanged. Re-ran every gate afterwards because those 47 assertions depend on it: `--mode ns` 790/34, `--mode s` 555/3, no-holdings 23/801, tool tests 111 passed against both roots — all identical to the pre-change run, so the separator changes no outcome. |
| 7 | Fenced block in this file had no language (markdownlint **MD040**) | **Applied** (`3323423`). Handled differently from finding 1 on purpose: MD040 costs one word and risks nothing, whereas the MD029 fix would renumber entries cited by number elsewhere. Cheap and safe is worth doing ahead of the gate; expensive and risky waits for PR-34. |
| 8 | The `--pprint` interpreter-coverage claim in this file was overstated | **Applied** (`323f656`). The claim that local 3.12 and 3.14 checks "span the 3.10-3.13 matrix" was wrong twice over: 3.14 is outside the matrix and 3.12 is the only point the two share with it, and 3.13 was equally unavailable locally, not just 3.10 and 3.11. Restated as a two-interpreter spot check with 3.10, 3.11 and 3.13 resting on CI alone. Worth fixing rather than waving through: `critiques/` is the evidence trail the §6.2 gate and the review loop depend on, and a record that overstates its own coverage is the same defect as a test that asserts less than it claims. |
| 9 | `ToolRun.__init__` is missing a `-> None` return annotation | **Rebutted.** This is an inline type annotation, which **ground rule 5** locks out for this entire effort ("Skip type annotations for now. No inline typing effort, no mypy gate. Provide a type stub for the public API instead"), and which §6.6's progressive-compliance schedule lists as **permanently waived** — `.pyi` stubs arrive at PR-35 and carry the typing, deliberately without touching implementation files. PR-23's spec independently names "would add an inline type annotation forbidden by ground rule 5" as a reason a lint violation is freeze-locked rather than fixable. No ruff rule in the configured select set flags this, so no gate is involved either. Recorded, not applied. |
