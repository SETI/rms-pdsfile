...
| m9 | §7's closing "no other log line changed" is overbroad: B1 changes which volume types a run walks, so it changes the `Volume types` line and the set of per-voltype events | **accepted, fixed.** A new subsection enumerates it — §7.7 when this round wrote it, renumbered §7.8 when round 4 inserted the command-line echoes ahead of it — and the closing claim is scoped to every subsection of §7 |
