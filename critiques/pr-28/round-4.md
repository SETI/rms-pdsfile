...
| M3 | The Phase 6 closure claimed **one** leftover question. Six more deferred entries still routed work at Phase 6 or at one of its PRs, and entry 72 named PR-28 itself as the nearest PR licensed to change behavior — a completed PR as a future owner, the exact defect round 2 raised | **Fixed.** The closure carries a seven-row table of everything the phase leaves behind, and says why they are listed rather than re-owned one by one. Entry 72 is re-owned as unowned, with the reason PR-28's licence did not reach `pdscache.py` |
