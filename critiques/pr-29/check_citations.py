...
CITATIONS = [
    ...
    ('src/pdsfile/pdscache.py', 141, 'self.keys = set()'),
    ('src/pdsfile/pdscache.py', 156, 'self.preload_eligible = True'),
    ('src/pdsfile/pdscache.py', 172, 'if len(self.keys) > self.limit + self.slop:'),
    ('src/pdsfile/pdscache.py', 173, 'expirations = [(self.dict[k][1], k)'),
    ('src/pdsfile/pdscache.py', 174, 'is not None]'),
    ('src/pdsfile/pdscache.py', 179, 'self.keys.remove(key)'),
    ('src/pdsfile/pdscache.py', 363, 'def get_multi'),
    ('src/pdsfile/pdscache.py', 383, 'value = self[key]'),
    ('src/pdsfile/pdscache.py', 448, 'if expiration:'),
    ('src/pdsfile/pdscache.py', 449, 'self.keys.add(key)'),
    ('src/pdsfile/pdscache.py', 532, 'del self.dict[key]'),
    ('src/pdsfile/pdscache.py', 552, 'del self.dict[key]'),
    ('src/pdsfile/pdscache.py', 572, 'del self.dict[key]'),
    ('src/pdsfile/pdscache.py', 593, 'self.keys = set()'),
    ('src/pdsfile/pdscache.py', 805, 'if blocking_pid in (0, self.pid):'),
    ('src/pdsfile/pdscache.py', 809, 'unblock_time = time.time() + MAX_BLOCK_SECONDS'),
    ('src/pdsfile/pdscache.py', 824, 'if test_pid != blocking_pid:'),
    ('src/pdsfile/pdscache.py', 830, 'broke a block'),
    ('src/pdsfile/pdscache.py', 891, 'was outraced by'),
    ('src/pdsfile/pdscache.py', 909, 'if not test_pid and self.logger:'),
    ('src/pdsfile/pdscache.py', 915, 'if test_pid != self.pid and self.logger:'),
    ('src/pdsfile/pdscache.py', 1113, 'keys = mydict.keys()'),
    ('src/pdsfile/pdscache.py', 1115, 'keys.sort()'),
    ('src/pdsfile/pdscache.py', 1123, 'len(self.local_keys_by_lifetime) - len(failures)'),
    ('src/pdsfile/pdscache.py', 1178, 'if key in self.permanent_values:'),
    ('src/pdsfile/pdscache.py', 1179, 'self._restore_permanent_to_cache()'),
    ('src/pdsfile/pdscache.py', 1215, 'def get_multi'),
    ('src/pdsfile/pdscache.py', 1267, 'for key in nonlocal_keys:'),
    ('src/pdsfile/pdscache.py', 1269, 'self._restore_permanent_to_cache()'),
    ('src/pdsfile/pdscache.py', 1270, 'break'),
    ('src/pdsfile/pdscache.py', 1463, 'if self.lifetime:'),
    ('src/pdsfile/pdscache.py', 1508, 'if key in self.permanent_values:'),
    ('src/pdsfile/pdscache.py', 1509, 'del self.permanent_values[key]'),
    ('src/pdsfile/pdscache.py', 1514, 'return status1 or status2'),
    ('src/pdsfile/pdscache.py', 1556, '_ = self.mc.del_multi(keys)'),
    ('src/pdsfile/pdscache.py', 1563, '_del_local'),
    ('src/pdsfile/pdscache.py', 1574, 'def _delete_local'),
    ('src/pdsfile/pdscache.py', 1571, 'count = len(self) - prev_len'),
    ('src/pdsfile/pdscache.py', 1632, "self.wait_for_unblock('clear')"),
    ('src/pdsfile/pdscache.py', 1634, "max(self.mc.get('$CLEAR_COUNT'), self.clear_count)"),
    ('src/pdsfile/pdscache.py', 1635, 'self.mc.flush_all()'),
    ('src/pdsfile/pdscache.py', 1680, 'lost from memcache'),
    ('src/pdsfile/pdscache.py', 1681, "self.mc.set('$CLEAR_COUNT', clear_count, time=0)"),
    ('src/pdsfile/pdscache.py', 1777, 'Permanent object is TooBig'),
...
def check_scope_table(problems):
    ...
    expected_lines = {'pdsfile.py': 2435, 'pdscache.py': 1782, 'pdsviewable.py': 986,
