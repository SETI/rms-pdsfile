...
CITATIONS = [
    ('src/pdsfile/pdscache.py', 1389, 'if self.lifetime:'),
    ('src/pdsfile/pdscache.py', 328, 'def get_multi'),
    ('src/pdsfile/pdscache.py', 348, 'value = self[key]'),
    ('src/pdsfile/pdscache.py', 1147, 'def get_multi'),
    ('src/pdsfile/pdscache.py', 1483, '_del_local'),
    ('src/pdsfile/pdscache.py', 1494, 'def _delete_local'),
    ('src/pdsfile/pdscache.py', 1491, 'count = len(self) - prev_len'),
    ('src/pdsfile/pdscache.py', 1045, 'keys = mydict.keys()'),
    ('src/pdsfile/pdscache.py', 1047, 'keys.sort()'),
    ('src/pdsfile/pdscache.py', 1055, 'len(self.local_keys_by_lifetime) - len(failures)'),
    ('src/pdsfile/pdscache.py', 765, 'broke a block'),
    ('src/pdsfile/pdscache.py', 826, 'was outraced by'),
    ('src/pdsfile/pdscache.py', 1686, 'Permanent object is TooBig'),
    ('src/pdsfile/pdscache.py', 842, 'if not test_pid and self.logger:'),
    ('src/pdsfile/pdscache.py', 848, 'if test_pid != self.pid and self.logger:'),
    ('src/pdsfile/pdscache.py', 1591, 'lost from memcache'),
    ('src/pdsfile/pdscache.py', 1592, "self.mc.set('$CLEAR_COUNT', clear_count, time=0)"),
    ('src/pdsfile/pdscache.py', 52, 'MEMCACHED_LOADED = True'),
...
def check_scope_table(problems):
    ...
    expected_lines = {'pdsfile.py': 2360, 'pdscache.py': 1691, 'pdsviewable.py': 954,
