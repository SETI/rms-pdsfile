...
``pds4checksums --infoshelf`` is accepted and chains nothing. Run the two in sequence:
...
and compared as a string, so a bundle shelved under one ``TZ`` and validated under
another disagrees with itself on every file.
