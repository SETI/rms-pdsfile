...
| `ruff check --preview --select E111…E117` (indentation) | **Active** (PR-24, owner 2026-08-04) | Every logical line on the 4-space grid, and comments aligned with the code they introduce. A separate invocation because the `E1` rules are preview-gated and enabling preview wholesale in `pyproject.toml` also changes the stable rules' behaviour — measured at 5,687 findings against a tree the configured gate reports clean. Naming the codes explicitly keeps the surface to indentation alone |
