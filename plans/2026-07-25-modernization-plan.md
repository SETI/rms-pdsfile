...
command line, and both are read by every other part of the subpackage.
  ...
  * The 15 per-tool modules under `src/pdsfile/holdings_maintenance/pds3/` and `pds4/`,
    ...
    shared behavior PR-30a has already described, the four that parse their own command
    lines, and `linkshelf_repairs.py`, which is a data table. It is done in two PRs
    rather than one.
  * **PR-30b (done)** — the five tool-family pairs and the repair table: 11 modules,
    4,681 lines, 74 functions, 48 of them undocumented, 218 parameters, and not one of
    the eleven with a module docstring. **177 of the 271 findings are here.** These are
    five near-identical pairs, so the defect a docstring PR is most exposed to is a
