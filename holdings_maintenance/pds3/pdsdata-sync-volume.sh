...
#########################################################################################
# Synchronize one volume from one pdsdata drive to another.
#
# Usage:
#   pdsdata-sync-volume [--dry-run] [--delete] <old> <new> <volset> <volume>
#
# Syncs the specified volume <volset/volume> from the drive /Volumes/pdsdata-<old> to the 
# drive /Volumes/pdsdata-<new>. Use the option "--dry-run" for a test dry run. Use the 
# option "--delete" to delete extraneous files in the remote directory. The rsync options 
# -a (archive) mode and -v (verbose) are included by default.
#
# Example:
#   pdsdata-sync-volume --delete admin staging GO_0xxx GO_0023
# copies all files relevant to the volume "GO_0xxx/GO_0023" from the drive pdsdata-admin 
# to the drive pdsdata-staging, deleting any extraneous files in the destination 
# directories.
#########################################################################################
...
do
  if [ -d /Volumes/pdsdata-${SRC}/holdings/${TYPE}/${VOLSET}/${VOLUME} ]; then
    ...
    rsync -av ${ARG1} ${ARG2} --include="${VOLUME}.tar.gz" \
    ...
    if [ -d /Volumes/pdsdata-${SRC}/holdings/_linkshelf-${TYPE}/${VOLSET} ]; then
      ...
      rsync -av ${ARG1} ${ARG2} --include="${VOLUME}_links.py" \
