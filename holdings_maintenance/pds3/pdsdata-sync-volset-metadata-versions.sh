...
#
# Usage:
#   pdsdata-sync-volset-metadata-versions [--dry-run] [--delete] <old> <new> <volset>
#
# Syncs the metadata for the specified volume set <volset> from the drive
# /Volumes/pdsdata-<old> to the drive /Volumes/pdsdata-<new>, including old versions. Use 
# the option "--dry-run" for a test dry run. Use the option "--delete" to delete 
# extraneous files in the remote directory. The rsync options -a (archive) mode and -v 
# (verbose) are included by default.This only syncs the metadata and associated 
# directories and should be used when syncing versioned metadata.
#
# Example:
#   pdsdata-sync-volset-metadata-versions --delete admin staging VGx_9xxx
