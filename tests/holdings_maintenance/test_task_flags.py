...
#
# PR-25 factors this parser into holdings_maintenance/_common.build_arg_parser().
# Its spec explicitly forbids introducing add_mutually_exclusive_group(), because
# that would turn today's silent last-wins into a hard argparse error -- an
# observable CLI behavior change. These tests are what makes that regression
# visible, so they exist for PR-25's benefit, not for their own.
#
# Only the argument parsing is under test here, so the tasks chosen are read-only
# ones and the tree is left alone.
##########################################################################################
...
def test_a_missing_task_is_an_error(tool_tree):
    """With no task flag at all the tool refuses to run."""

    run = support.run_tool(tool_tree, 'pdsarchives', tool_tree.path(VOLUME_DIR))
...
def test_two_task_flags_resolve_to_the_last_one(tool_tree, flags, expected):
    """Multiple task flags are accepted; the rightmost one silently wins.

    This is today's behavior and PR-25 must preserve it. If the shared parser ever
    grows a mutually exclusive group, argparse will exit 2 with "not allowed with
    argument" and these cases will fail -- which is the point.
    """

    run = support.run_tool(tool_tree, 'pdsarchives', *flags,
...
def test_another_pds3_tool_resolves_task_flags_the_same_way(tool_tree):
    ...
    both parse the flags the same way. The pds4 half of the pair is covered by
    test_pds4_checksums.test_two_task_flags_resolve_to_the_last_one, which has a
    pds4 tree to point at.
    """

    run = support.run_tool(tool_tree, 'pdschecksums', '--validate', '--update',
                           tool_tree.path(VOLUME_DIR))
    assert 'not allowed with argument' not in run.output, run.describe()
    assert task_announced(run) == 'update', run.describe()


def test_short_and_long_aliases_select_the_same_task(tool_tree):
    """--init is an alias of --initialize, and --reinit of --reinitialize."""

    run = support.run_tool(tool_tree, 'pdsarchives', '--init',
                           tool_tree.path(VOLUME_DIR))
    assert task_announced(run) == 'initialize', run.describe()

    run = support.run_tool(tool_tree, 'pdsarchives', '--reinit',
                           tool_tree.path(VOLUME_DIR))
