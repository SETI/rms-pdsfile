...
#   * crlf is driven in-process, through support.run_tool_in_process(). It imports
#     no PdsFile class and reads neither holdings root -- it reads exactly the
#     paths on its command line -- so the hazard above cannot arise.
#     support.HOLDINGS_FREE_TOOLS is the list, and both helpers assert against it.
#     It still keeps two subprocess tests, through
#     support.run_tool_without_holdings(), because only a subprocess can show that
#     `python -m <module>` reaches main(), that the process exit code is what
#     main() returned, and what an uncaught exception leaves the process at.
...
#     bookkeeping is pinned by test_update_holdings_script.py: it parses the
#     script's text, and runs the script only far enough to prove each argument
#     guard exits 1 before the first deletion -- never past the guards.
##########################################################################################
