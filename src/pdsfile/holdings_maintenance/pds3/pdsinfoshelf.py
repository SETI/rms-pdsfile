...
because it picks between the volume and the volume set log path per target instead. And
...
def generate_infodict(pdsdir, selection, old_infodict={}, *, logger=None,
    ...
    **A directory's entry is the sum of what is under it.** Its byte count is the total of
    ...
    def get_info_for_file(abspath):
        ...
        and shelved as (0, 0). **The image is closed only where nothing was raised**, so a
        file that fails after being opened stays open until it is collected.
...
def load_infodict(pdsdir, *, logger=None, limits=None):
    ...
    The shelf stores each key as the path below the volume set, so reading it is the
    inverse of what ``write_infodict()`` does: the volume's own prefix is put back in
    front of each. The empty key is the volume directory itself and is restored as such.
...
def write_infodict(pdsdir, infodict, *, logger=None, limits=None):
    ...
    path, keyed by the path below the volume set rather than by absolute path, and a
    ...
    parts** of the file's basename with "_info" appended, so a shelf file whose basename
    has fewer than two such parts is named for what there is, and one with more loses the
    rest. The sidecar is written as latin-1.
...
def update(pdsdir, selection=None, logger=None, limits=None):
    ...
    size has changed. That is what makes an update of a volume that has gained a file
