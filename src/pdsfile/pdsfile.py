...
# pdstable is used by the index-row methods, which live in _index_rows.py, and is
# a frozen member of this module's surface, so it is bound here in the same
# redundant-alias form as the seven above.
...
# Four groups of PdsFile methods live in private modules as mixins, and the class
...
# module's public surface; _GLOB_CACHE_SIZE and _clean_glob are private, and are
# carried so that no name reachable as pdsfile.pdsfile.<name> is lost. The
...
class PdsFile(_DerivedPathsMixin, _IndexRowsMixin, _LocalFsMixin, _ShelfMixin,
              object):
